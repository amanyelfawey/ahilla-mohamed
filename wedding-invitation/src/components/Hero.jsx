...
const SENTENCE = "The day we've been dreaming of is almost here — and it would not be complete without you."
    ...
    <span className="inline-flex flex-wrap justify-center">
      {Array.from(text).map((char, i) => (
        <motion.span
          ...
          initial={reduced ? { opacity: 0 } : { opacity: 0, y: '0.45em', rotateX: -75 }}
          animate={
            opened
          ...
          transition={{ duration: 0.95, ease: EASE_LUXE, delay: baseDelay + i * 0.05 }}
        >
          {char === ' ' ? '\u00A0' : char}
        </motion.span>
      ))}
...
export function Hero({ opened }) {
  ...
  const archY0 = useTransform(smoothScroll, [0, 1], [GALLERY[0].lift, GALLERY[0].lift + GALLERY[0].depth])
  const archY1 = useTransform(smoothScroll, [0, 1], [GALLERY[1].lift, GALLERY[1].lift + GALLERY[1].depth])
  const archY2 = useTransform(smoothScroll, [0, 1], [GALLERY[2].lift, GALLERY[2].lift + GALLERY[2].depth])
  const archY3 = useTransform(smoothScroll, [0, 1], [GALLERY[3].lift, GALLERY[3].lift + GALLERY[3].depth])
  const archY = [archY0, archY1, archY2, archY3]

  const entry = (delay) => ({
    initial: reduced ? { opacity: 0 } : { opacity: 0, y: 22, filter: 'blur(5px)' },
    animate: opened
  ...
  return (
    ...
    >
        ...
        <div className="pointer-events-none absolute inset-0 z-[1] overflow-hidden" aria-hidden="true">
          {DUST.map((mote) => (
            <motion.span
              ...
              animate={{ y: [0, -760], x: [0, mote.drift, 0], opacity: [0, 0.7, 0] }}
              transition={{ duration: mote.duration, repeat: Infinity, ease: 'linear', delay: mote.delay }}
            />
          ))}
        </div>
      )}
      ...
      >
        {/* Date & venue */}
        <motion.div className="flex flex-col items-center gap-3" {...entry(0.2)}>
        <p className="font-script text-[1.35rem] leading-none text-gold-deep/90"> With hearts full of love and happiness </p> <p className="font-script text-[1.6rem] leading-none text-gold-deep"> Ahilla & Mohamed </p> <p className="font-script text-[1.35rem] leading-none text-gold-deep/90"> invite you to be part of their forever </p>
        ...
        <div className="grid w-full max-w-[880px] grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-5">
          {GALLERY.map((item, i) => (
            <motion.figure
              ...
              style={{ y: archY[i] }}
              initial={reduced ? { opacity: 0 } : { opacity: 0, y: 64, scale: 0.9, rotate: item.tilt * 2 }}
              animate={
                opened
              ...
              whileHover={reduced ? undefined : { y: -12, scale: 1.03, rotate: 0 }}
            >
              <div className="relative overflow-hidden rounded-t-[999px] rounded-b-[10px] border border-gold/30 bg-cream shadow-[0_26px_64px_-26px_rgba(120,90,95,0.55)]">
                ...
                {!reduced && (
                  <motion.div
                    ...
                    transition={{ duration: 1.5, ease: EASE_LUXE, delay: item.delay + 0.5 }}
                  />
                )}
              </div>
            </motion.figure>
        ...
        {/* Headline */}
        <motion.div className="relative flex flex-col items-center gap-5" {...entry(1.05)}>
          <h1 className="relative font-display text-[clamp(3rem,11.5vw,6.5rem)] font-extralight leading-[0.95] tracking-[0.01em] text-ink">
            <KineticHeadline text={HEADLINE} opened={opened} reduced={reduced} baseDelay={1.15} />
            {!reduced && (
              <motion.span
                ...
                transition={{ duration: 4.5, repeat: Infinity, ease: 'linear', repeatDelay: 2.6 }}
